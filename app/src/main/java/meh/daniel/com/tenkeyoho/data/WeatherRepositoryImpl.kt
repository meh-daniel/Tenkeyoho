...
class WeatherRepositoryImpl(
    ...
    override suspend fun getWeather(): WeatherOfCity {
        return try {
            val weathers = weatherApi.getWeatherNWByCity(appid = APP_ID, city = CITY, units = UNITS, lang = LANG).toDomain()
            appDataBase.weatherDao().updateAllWeathersSWWithCity(weathers.weathers.toSW(), CitySW(name = weathers.nameCity))
            weathers
        }
        catch (e: Exception) {
